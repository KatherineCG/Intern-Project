...
class BugzillaServer(object):
    ...
    def original_bugdata(self, bugid):
        return self.server.Bug.show_bug(bugid)

    def show_bug(self, bug_id, bugdata):
        try:
        ...
        for column in columns:
            ...
            if column['name'] == "comments":
                ...
                or self.options.empty):
                    ...
                    bugdata[column['description']] = bug[column['name']]
        return bugdata
...
def main(args):
    ...
    if (options.query):
        bugidlist = server.query(options.query)
        sys.exit(0)
